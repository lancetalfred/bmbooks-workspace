...
|---|---|
| You add a new book | It appears on the website within 1 hour |
...
| You change a book's status to **OP** (out of print), **RP** (reprinting), or **RUC** (reprint under consideration) | It disappears from the website within 1 hour |
...
If a book has no copies on the shelf, it doesn't disappear from the website. Instead, it shows as **"Available to order"** — customers can still buy it, and you'll order it in when the order comes through. This is exactly how the current website works.

---

## The 1-hour window

The website isn't updated in real time — it updates every hour. This means there's a short window where, for example, a book sold in-store might still show as "in stock" online. In practice this is rarely a problem, but it's worth knowing.

If you ever need to trigger an update immediately — for example, you've just done a big stock change and want the website to reflect it now — double-click the **Sync Now** shortcut on the shop computer desktop. The BookKeeper window will open showing live progress, a count of what's been updated, and any errors. You can also click the **Sync Now** button inside the window at any time.
...
| Activate new products (set Draft → Active) | Products | When new books are added |
...
Lance is building Phase 2 which will do this automatically — once that's live, Shopify orders will flow into Bookscan's web order tables without any manual entry.

**In the meantime, when you get an online order:**
...
Contact Lance: lalfred@linkedin.com
